...
export default function Progress() {
  ...
  const measurements = Object.entries(progress)
    .filter(([_, data]: [string, any]) => data.weight || data.waist || data.hip)
    .sort(([a], [b]) => new Date(b).getTime() - new Date(a).getTime());

  const latestMeasurement = measurements[0]?.[1] || {};

  // Calculate changes from initial
  const calculateChange = (current: number | undefined, initial: number | undefined) => {
    if (!current || !initial) return null;
    return current - initial;
  };

  const weightChange = calculateChange(
    latestMeasurement.weight || profile?.weight,
    profile?.weight
  );
  const waistChange = calculateChange(
    latestMeasurement.waist || profile?.waist,
    profile?.waist
  );
  const hipChange = calculateChange(
    latestMeasurement.hip || profile?.hip,
    profile?.hip
  );

  // Calculate completed days
  const completedDays = Object.values(progress).filter((day: any) => {
    if (!day.checks) return false;
    return ['recipe', 'exercise', 'water', 'sugar'].every(key => day.checks[key]);
  }).length;

  const handleAddMeasurement = () => {
    const newData: any = { date: today };
    
    if (newMeasurement.weight) newData.weight = parseFloat(newMeasurement.weight);
    if (newMeasurement.waist) newData.waist = parseFloat(newMeasurement.waist);
    if (newMeasurement.hip) newData.hip = parseFloat(newMeasurement.hip);
        ...
        <div className="bg-card rounded-2xl p-5 shadow-card border border-border">
          ...
          {showAddMeasurement && (
            <div className="mb-4 p-4 bg-muted/50 rounded-xl space-y-3">
              ...
              <Button variant="warm" className="w-full" onClick={handleAddMeasurement}>
                Guardar Medidas
              </Button>
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold text-foreground">
                    {latestMeasurement.weight || profile?.weight || '—'} kg
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                  'text-sm font-medium',
                  weightChange && weightChange < 0 ? 'text-sage-dark' : 
                  weightChange && weightChange > 0 ? 'text-primary' : 'text-muted-foreground'
                )}>
            ...
            <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold text-foreground">
                    {latestMeasurement.waist || profile?.waist || '—'} cm
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                  'text-sm font-medium',
                  waistChange && waistChange < 0 ? 'text-sage-dark' : 
                  waistChange && waistChange > 0 ? 'text-destructive' : 'text-muted-foreground'
                )}>
                  {formatChange(waistChange, 'cm')}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold text-foreground">
                    {latestMeasurement.hip || profile?.hip || '—'} cm
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                  'text-sm font-medium',
                  hipChange && hipChange > 0 ? 'text-sage-dark' : 
                  hipChange && hipChange < 0 ? 'text-destructive' : 'text-muted-foreground'
                )}>
                  {formatChange(hipChange, 'cm')}
                </span>
              </div>
            </div>
          </div>
        ...
        {measurements.length > 0 && (
          <div className="bg-card rounded-2xl p-5 shadow-card border border-border">
            ...
            <div className="space-y-3">
              {measurements.slice(0, 5).map(([date, data]: [string, any]) => (
